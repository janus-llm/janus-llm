def ZERO(PSSIEN,PSSFT,PSSFL,LIST):
    ...
    def DRGIEN(PSS50,PSSFL,LIST):
        ...
        def INACTDT(PSSIEN):
            ...
            with tempfile.NamedTemporaryFile(suffix=".m") as temp:
                temp.write(script.encode())
                temp.seek(0)
                cmd = f'mumps -run {temp.name}'
                output = subprocess.check_output(cmd, shell=True).decode().strip()
                return output

        def LOOKUP(PSSFT,LIST):
            """
            PSSFT - Free Text name in IV SOLUTIONS file (#52.7).
            LIST - Subscript of ^TMP array in the form ^TMP($J,LIST,Field Number where Field Number is the
                   Field Number of the data piece being returned.
            ...
            LOOKUP(PSSFT,LIST) ;
             ...
             D FIND^DIC(52.7,,"@;.01;2","QP",PSSFT,,"B",,,"")
             I +$G(^TMP("DILIST",$J,0))=0 S ^TMP($J,LIST,0)=-1_"^"_"NO DATA FOUND" Q
             I +^TMP("DILIST",$J,0)>0 S ^TMP($J,LIST,0)=+^TMP("DILIST",$J,0) N PSSXX S PSSXX=0 F  S PSSXX=$O(^TMP("DILIST",$J,PSSXX)) Q:'PSSXX  D
        ...
        def SETSCRN():
            """
            Set Screen for inactive Solutions
            Naked reference below refers to ^PS(52.7,+Y,"I")
            """
            import os
            import tempfile
    ...
    # Call the MUMPS functions
    output = ZERO(PSSIEN, PSSFT, PSSFL, LIST)
    output = DRGIEN(PSS50, PSSFL, LIST)
    output = INACTDT(PSSIEN)
    output = LOOKUP(PSSFT, LIST)
    output = POICHK(PSSIEN)
    output = POI(PSSOI, PSSFL, LIST)
    output = ACTSOL(PSSFL, LIST)