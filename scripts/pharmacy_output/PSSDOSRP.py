def PSSDOSRP():
    # BIR/RTR-Dosage review report ;03/24/00
    ...
    DIR = {}
    DIR['0'] = "S^A:ALL;S:SELECT A RANGE"
    DIR['B'] = "S"
    DIR['A'] = "Print Report for (A)ll or (S)elect a Range"
    # D ^DIR K DIR I Y["^"!($D(DTOUT))!($D(DUOUT)) W !!,"Nothing queued to print.",! G ENDX
    Y = ''
    if Y["^" or 'DTOUT' in globals() or 'DUOUT' in globals():
    ...
    # S DIR("A")="Print Report for (A)ll or (S)elect a Range",DIR(0)="S^A:ALL;S:SELECT A RANGE"
    # D ^DIR K DIR I Y["^"!($D(DTOUT))!($D(DUOUT)) W !!,"Nothing queued to print.",! G ENDX
    DIR['A'] = "Print Report for (A)ll or (S)elect a Range"
    DIR['0'] = "S^A:ALL;S:SELECT A RANGE"
    # D ^DIR K DIR I Y["^"!($D(DTOUT))!($D(DUOUT)) W !!,"Nothing queued to print.",! G ENDX
    ...
    while PSSNUMB != None and PSSNUMBX == None:
        # S PSSNUMBX=1
        PSSNUMBX = 1
    ...
    if PSSNUMBX and Y == 1:
        ...
        # S DIR("A")="Print report for drugs with leading numerics",DIR(0)="Y"
        # D ^DIR K DIR I Y["^"!($D(DUOUT))!($D(DTOUT)) W !!,"Nothing queued to print.",! G ENDX
        DIR['A'] = "Print report for drugs with leading numerics"
        DIR['0'] = "Y"
        # D ^DIR K DIR I Y["^"!($D(DUOUT))!($D(DTOUT)) W !!,"Nothing queued to print.",! G ENDX
        Y = ''
        if Y["^" or 'DTOUT' in globals() or 'DUOUT' in globals():
    ...
    # DIR("A")="Select a Range",DIR(0)="F^1:3" D ^DIR I Y["^"!($D(DTOUT))!($D(DUOUT)) W !!,"Nothing queued to print.",! G ENDX
    DIR['?'] = ' '
    DIR['?'][1] = ' '
    DIR['?'][2] = ' '
    DIR['?'][3] = ' '
    DIR['?'][4] = ' '
    DIR['?'] = ' '
    DIR['A'] = "Select a Range"
    DIR['0'] = "F^1:3"
    # D ^DIR I Y["^"!($D(DTOUT))!($D(DUOUT)) W !!,"Nothing queued to print.",! G ENDX
    Y = ''
    if Y["^" or 'DTOUT' in globals() or 'DUOUT' in globals():
        print("\nNothing queued to print.\n")
        return
    ...
    X = Y
    if X != 1U and X != 1U1"-"1U and X != 1L and X != 1L1"-"1L:
    ...
    IOP = ''
    %ZIS = "QM"
    # D ^%ZIS I $G(POP) W !!,"Nothing queued to print.",! G ENDX
    if 'POP' in globals():
        print("\nNothing queued to print.\n")
        return
    ...
    if 'IO' in globals():
        ZTRTN = "START^PSSDOSRP"
        ZTDESC = "Dosage Review Report"
        ZTSAVE = {}
        ZTSAVE['PSSHOW'] = ''
        ZTSAVE['PSSBEG'] = ''
        ZTSAVE['PSSEND'] = ''
        ZTSAVE['PSSSRT'] = ''
        # D ^%ZTLOAD K %ZIS W !,"Report queued to print.",! G ENDX
    ...
    while True:
        # S PSSNAME=$O(^PSDRUG("B",PSSNAME))
        PSSNAME = None
        
        # Q:$S(PSSSRT="N"&('PSSNAME):1,PSSSRT="X"&(PSSNAME](PSSEND_"zzzz")):1,1:0)!(PSSNAME=""&(PSSSRT="X"))!(PSSSRT="A"&(PSSNAME=""))!($G(PSSOUT))
        if (PSSSRT == "N" and not PSSNAME) or (PSSSRT == "X" and PSSNAME > PSSEND + "zzzz") or (PSSNAME == "" and PSSSRT == "X") or (PSSSRT == "A" and PSSNAME == "") or 'PSSOUT' in globals():
            break
        
        # F PSSIEN=0:0 S PSSIEN=$O(^PSDRUG("B",PSSNAME,PSSIEN)) Q:'PSSIEN!($G(PSSOUT))  D
        PSSIEN = 0
        while True:
            # S PSSIEN=$O(^PSDRUG("B",PSSNAME,PSSIEN))
            PSSIEN = None
            
            # Q:'PSSIEN!($G(PSSOUT))
            if not PSSIEN or 'PSSOUT' in globals():
                break
            
            # Q:'$D(^PSDRUG(PSSIEN,0))
            if not '^PSDRUG'(PSSIEN,0):
                continue
            
            # I ($Y+5)>IOSL D HD Q:$G(PSSOUT)
            if ($Y + 5) > 'IOSL':
                HD()
                if 'PSSOUT' in globals():
                    break
