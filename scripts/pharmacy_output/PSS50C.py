def PSS50C(PSSIEN, PSSFT, PSSFL, PSSPK, PSSRTOI, LIST):
    """
...
def NOCMOP(PSSIEN2, PSSFL2):
    ...
    NDNODE = $G(^PSDRUG(+PSSIEN2,"ND"))
    INODE = $G(^("I"))
    NODE3 = $G(^(3))
    NODE2 = $G(^(2))
    ZNODE = $G(^(0))
    
    if $P(NODE2,"^",3)["O" and $P(NDNODE,"^",2)]"" and INODE="" and ($G(PSSFL2)=1 and NODE3=0) or (not '$D(^PSDRUG(+PSSIEN2,3))):
        return 1
