        ...
        for filename in files:
            if filename.endswith(".json"):
                ...
                for index, entry in enumerate(data.get("output", [])):
                    ...
                    # Keeps the old file name but adds a '_{index}' at the end
                    # for each individual `code` + `requirement` pairing
                    new_filename = f"{os.path.splitext(filename)[0]}_{index}.json"
                    new_file_path = os.path.join(output_path, new_filename)

                    new_data = {"code": code, "requirements": requirements}
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "input_directory", type=str, help="The directory containing JSON files to process"
    )
    parser.add_argument(
        "output_directory",
        type=str,
        help="The directory to save processed JSON files with preserved structure",
    )
