    ...
    for filename in os.listdir(directory):
        if filename.endswith(".json"):
            ...
            for index, entry in enumerate(data.get("output", [])):
                ...
                # keeps the  old file name but will add a '_{index}' at the end
                # for each individual `code` + `requirement` pairing: exampleOutput_0.json
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "directory", type=str, help="The directory containing JSON files to process"
    )
